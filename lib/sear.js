...
Sear.prototype.getExpressApp = function (options, ready) {
  ...
  var app = express();
  var fileWatcher = app.fileWatcher = new FileWatcher(this, options);
  ...
  if (!options.use_release) {
    ...
    if (options.live_update) {
      this.updateServer = app.updateServer = new UpdateServer(this, fileWatcher, options.server || server, options);
    }
