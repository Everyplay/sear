...
module.exports = function (files, options, callback) {
  ...
  function runTests() {
    ...
    if (options.minify_options) {
      options.minify_options.optimize_amd = false;
    }
    options.includes = options.includes || [];

    options.includes = _.union(requiredIncludes, options.includes);

    self.commands.build([], options, function (err, bundle) {
      if (err) {
        return callback(err);
      }

      var module = bundle.modules[0];
      // TODO do this for other modules also
      ...
      var pathToMochaPhantomJs = path.join(require.resolve('mocha-phantomjs'), '../../bin/mocha-phantomjs');
      var mocha = spawn(pathToMochaPhantomJs,
        args);
